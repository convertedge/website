...
const shortifyInputs = [
  ...
  { tag: "input", selector_name: "id", selector_value: "linkedin", error: "field", selected: "shortify", field: "linkedin_url" },
];
</script>

<template>
  <div class="max-w-7xl py-10 md:py-16 flex flex-col lg:flex-row">
    <div class="flex flex-col gap-2 md:gap-4">
      <div class="mb-2 md:mb-0">
        <h3 class="font-medium sm:text-lg text-gray-800">Step 1</h3>
        <p class="text-gray-600 text-sm">Enter page URL containing the form to shorten</p>
      </div>

      <div class="bg-gray-50 ring-1 ring-gray-200 rounded-md p-2">
        <UInput required name="domain" placeholder="https://apple.com/book-demo" class="mb-2 min-w-40 xl:min-w-60 w-full" :disabled="true">
          <template #leading>
    ...
    <div class="lg:-rotate-90 flex justify-center items-center">
      <svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="-5.0 -10.0 110.0 135.0" class="size-28 lg:size-20 xl:size-28">
        <path
    ...
    <div class="flex flex-col gap-2 md:gap-4">
      ...
      <div class="space-y-2 bg-gray-50 ring-1 ring-gray-200 p-2 rounded-md">
        <span class="grid grid-cols-2 sm:grid-cols-3 gap-x-4" v-for="(item, index) of shortifyInputs" :key="index">
          <p class="col-span-2 sm:col-span-1 text-sm sm:text-base">{{ item.selector_value }}:</p>
          <USelectMenu :options="['', 'shortify', 'enrich']" :disabled="true" v-model="item.selected" />
          <USelectMenu
    ...
    <div class="flex flex-col gap-2 md:gap-4">
      ...
      <div class="bg-gray-50 ring-1 ring-gray-200 p-2 rounded-md">
        <pre class="p-2 overflow-x-scroll text-xs sm:text-sm lg:max-w-68 xl:max-w-80">
&lt;script&gt;
